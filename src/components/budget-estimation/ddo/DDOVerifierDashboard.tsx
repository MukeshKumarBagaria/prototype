...
import { Search, Filter, FileText, Clock, Check } from 'lucide-react';
import Link from 'next/link';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { StatusBadge } from '@/components/budget-estimation/shared/StatusBadge';
import { MOCK_ESTIMATIONS, getBudgetLineItemById, formatCurrency } from '@/data/budget-estimation/mockData';

export function DDOVerifierDashboard() {
    const [searchTerm, setSearchTerm] = useState('');

    // Get estimations under verification
    const verifierEstimations = MOCK_ESTIMATIONS.filter(est =>
        est.currentLevel === 'ddo_verifier' || est.status === 'under_verification'
    );
    ...
    const stats = [
        {
            ...
            value: verifierEstimations.length,
            color: 'bg-amber-100 text-amber-600'
        },
        {
            icon: Clock,
            label: 'Today',
            value: verifierEstimations.filter(e => new Date(e.submittedAt || '').toDateString() === new Date().toDateString()).length,
    ...
    const filteredEstimations = verifierEstimations.filter(est => {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                ...
                <Link href="/budget-estimation/ddo-verifier/grid">
                                    ...
                                    <tr key={estimation.id} className="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <p className="text-sm text-slate-500">
                                                {estimation.submittedAt ? new Date(estimation.submittedAt).toLocaleString() : '-'}
                                            </p>
                                        </td>
                                        <td className="px-6 py-4">
                                            <Link href={`/budget-estimation/ddo-verifier/review/${estimation.id}`}>
