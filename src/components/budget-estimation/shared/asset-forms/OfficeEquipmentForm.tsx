...
import { OfficeEquipmentAsset } from '@/data/budget-estimation/types';

interface OfficeEquipmentFormProps {
    asset: OfficeEquipmentAsset;
    onChange: (asset: OfficeEquipmentAsset) => void;
    disabled?: boolean;
}

export function OfficeEquipmentForm({ asset, onChange, disabled = false }: OfficeEquipmentFormProps) {
    const handleChange = (field: keyof OfficeEquipmentAsset, value: any) => {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="md:col-span-3">
                <Label className="text-slate-700 font-medium">Name of Office Equipment <span className="text-red-500">*</span></Label>
                <Input
                    value={asset.equipmentName}
                    onChange={(e) => handleChange('equipmentName', e.target.value)}
                    disabled={disabled}
                    placeholder="e.g., Printer, Scanner, UPS, etc."
                    className="mt-2"
                />
            </div>
