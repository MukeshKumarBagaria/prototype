...
import { TelephoneAsset } from '@/data/budget-estimation/types';

interface TelephoneFormProps {
    asset: TelephoneAsset;
    onChange: (asset: TelephoneAsset) => void;
    disabled?: boolean;
}

export function TelephoneForm({ asset, onChange, disabled = false }: TelephoneFormProps) {
    const handleChange = (field: keyof TelephoneAsset, value: any) => {
        const updated = { ...asset, [field]: value };
        // Auto-calculate yearly total
        if (field === 'oldQuantity' || field === 'newRequired' || field === 'monthlyCharge') {
            const totalQty = (field === 'oldQuantity' ? value : updated.oldQuantity) +
                (field === 'newRequired' ? value : updated.newRequired);
            const monthly = field === 'monthlyCharge' ? value : updated.monthlyCharge;
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                <Label className="text-slate-700 font-medium">Type of Connection <span className="text-red-500">*</span></Label>
                <select
                    value={asset.connectionType}
            ...
            <div>
                <Label className="text-slate-700 font-medium">Monthly Charge (₹) <span className="text-red-500">*</span></Label>
                <Input
                    type="number"
                    min="0"
                    step="0.01"
                    value={asset.monthlyCharge}
                    onChange={(e) => handleChange('monthlyCharge', parseFloat(e.target.value) || 0)}
                    disabled={disabled}
                    className="mt-2"
                />
            </div>
            ...
            <div className="md:col-span-3">
                ...
                <Textarea
                    ...
                    disabled={disabled}
                    placeholder="Justify why additional connections are needed..."
                    className="mt-2"
                    rows={2}
                />
            </div>
