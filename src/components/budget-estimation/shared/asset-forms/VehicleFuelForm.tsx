...
import { VehicleFuelAsset } from '@/data/budget-estimation/types';

interface VehicleFuelFormProps {
    asset: VehicleFuelAsset;
    onChange: (asset: VehicleFuelAsset) => void;
    disabled?: boolean;
}

export function VehicleFuelForm({ asset, onChange, disabled = false }: VehicleFuelFormProps) {
    const handleChange = (field: keyof VehicleFuelAsset, value: any) => {
        const updated = { ...asset, [field]: value };
        // Auto-calculate yearly total
        if (field === 'oldQuantity' || field === 'newRequired' || field === 'monthlyFuelCost') {
            const totalQty = (field === 'oldQuantity' ? value : updated.oldQuantity) +
                (field === 'newRequired' ? value : updated.newRequired);
            const monthly = field === 'monthlyFuelCost' ? value : updated.monthlyFuelCost;
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                <Label className="text-slate-700 font-medium">Monthly Fuel Cost (₹) <span className="text-red-500">*</span></Label>
                <Input
                    type="number"
                    min="0"
                    step="0.01"
                    value={asset.monthlyFuelCost}
                    onChange={(e) => handleChange('monthlyFuelCost', parseFloat(e.target.value) || 0)}
                    disabled={disabled}
                    className="mt-2"
                />
            </div>
            ...
            <div className="md:col-span-3">
                <Label className="text-slate-700 font-medium">Reason for Additional Requirement <span className="text-red-500">*</span></Label>
                <Textarea
                    value={asset.reason}
                    onChange={(e) => handleChange('reason', e.target.value)}
                    disabled={disabled}
                    placeholder="Justify why additional vehicles/fuel is needed..."
                    className="mt-2"
                    rows={2}
                />
            </div>
